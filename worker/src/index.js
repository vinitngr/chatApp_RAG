  ...
  async fetch(req, env) {
    ...
    if (path.length === 2 && path[1] === "analytics") {
      if (url.searchParams.get("get") !== null || req.method === "GET") {
        ...
        const data = raw ? JSON.parse(raw) : { aiquery: 0, contentfetched: 0, tokenused: 0 };
      ...
      if (req.method === "POST") {
        ...
        if (!type) {
          return new Response("Missing 'type' query param", { status: 400 });
        }
        ...
        if (type === "aiquery") {
          data.aiquery++;
          const token = parseInt(url.searchParams.get("token") || "0", 10);
          if (!isNaN(token)) data.tokenused += token;
        } else if (type === "contentfetched") {
          data.contentfetched++;
        } else if (type === "tokenused") {
          const token = parseInt(url.searchParams.get("token") || "0", 10);
          if (!isNaN(token)) data.tokenused += token;
        } else {
